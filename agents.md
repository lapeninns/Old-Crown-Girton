...
5. **Prefer existing patterns** (DRY/KISS/YAGNI). **Use DaisyUI UI** (via **DaisyUI MCP**) before custom components.
...
| **2. Design & Planning**         | Architecture, contracts, UX states, tests, rollout | `plan.md`                     | **DaisyUI**, **Supabase**, **Next DevTools** | **Frontend Aesthetics**, **Style Principles**, **MCP Integration** |
| **3. Implementation**            | Code, migrations, components, unit tests           | `todo.md` (live)              | **DaisyUI**, **Supabase**, **Next DevTools** | **Style Principles**, **MCP Integration**, **Continuity Ledger**   |
...
- **MCP Integration** (skills/mcp-integration.md) — Use **DaisyUI** for components; **Supabase** for remote migrations (dry‑run → plan → apply); **Next DevTools** for routing/bundle inspection.

**Goal**: Turn analysis into an implementable blueprint.

**Activities**:

- Mobile‑first, progressive enhancement; prefer existing components (DaisyUI).
...
- **MCP Integration** (skills/mcp-integration.md) — Use **DaisyUI** to scaffold, **Next DevTools** to optimize, **Supabase** for migrations.
...
- [ ] Create/extend components (DaisyUI-first; exception noted if any)
...
- **Use DaisyUI UI via DaisyUI MCP**; extend rather than rebuild.
- **Exceptions**: Only if no DaisyUI equivalent supports required a11y/UX; document justification in `plan.md` and get design sign‑off.
...
- **DaisyUI MCP** — Discover/scaffold UI components, synchronize tokens.  
  **Phases**: 2, 3. **Rule**: Prefer DaisyUI before custom.
